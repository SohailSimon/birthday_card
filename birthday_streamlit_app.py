...
st.title("🎉❤️ A Speical Present ❤️🎉")

# ---- Show present image wrapped in a button if not opened ----
if not st.session_state.opened:
    st.markdown("<h3>Unwrap the present to see what you got!</h3>", unsafe_allow_html=True)
    
    # Button with the image inside
    clicked = st.button("Unwrap Me 🎁", key="present_button")  # hidden label
    st.markdown(
        """
        <form action="" method="get">
            <button type="submit" class="present-button">
                <img src="https://cdn.pixabay.com/photo/2020/10/27/15/14/gift-5691042_1280.png" width="250">
            </button>
        </form>
        """,
        unsafe_allow_html=True
...
if st.session_state.opened:
    ...
    # Birthday message
    st.markdown(
        f"<h1>Happy Birthday, {birthday_name}! 🥳💖 Have an amazing Birthday, with many more to come! ❤️ </h1>",
        unsafe_allow_html=True
    )
